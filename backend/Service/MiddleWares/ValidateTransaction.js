...
const validate_transaction = (req,res,next)=>{
    ...
    if(transactions.expiry && today>=expiry){
        return res.status(400).send({error : "your card has expired"});
    }
    if(transactions.card_number && transactions.card_number.length <10 ){
        return res.status(400).send({error : "invalid card number"});
    }
    if( transactions.pin &&  transactions.pin < 1000 || transactions.pin >9999 ){
